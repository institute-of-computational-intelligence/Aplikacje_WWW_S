...
namespace SchoolRegister.Web.Controllers {
    ...
    public class TeacherController : BaseController {
        ...
        public IActionResult AddGrade (AddGradeVm AddGradeVm) {
            try {
                if (ModelState.IsValid) {
                    var teacher = _userManager.GetUserAsync (User).Result;
                    AddGradeVm.TeacherId = teacher.Id;
                    _gradeService.AddGradeToStudent (AddGradeVm);
                    return RedirectToAction ("Details", "Student", new { studentId = AddGradeVm.StudentId });
                }
            } catch (Exception ex) {
