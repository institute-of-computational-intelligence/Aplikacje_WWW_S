    ...
    public class GradeApiController : BaseApiController
    {
        ...
        public async Task<IActionResult> AddGrade([FromBody] AddGradeToStudentVm addGradeToStudentVm)
        {
            try
            {
                ...
                var addGradeVm = await _teacherService.AddGradeToStudent(addGradeToStudentVm);
        ...
        public async Task<IActionResult> ViewGrade([FromQuery] GradesVm gradesVm)
        {
            try
            {
                if (ModelState == null || !ModelState.IsValid)
                    return BadRequest(ModelState);
                var getGradesResult = await _gradeService.ShowGrades(gradesVm);
