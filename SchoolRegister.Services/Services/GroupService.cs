...
namespace SchoolRegister.Services.Services
{
    public class GroupService : BaseService, IGroupService
    {
        ...
        public async void DeleteGroupAsync(RemoveGroupVm removeGroupVm)
        {
            try
            {
                var group = await DbContext.Groups.FirstOrDefaultAsync(g => g.Id == removeGroupVm.Id);

                if (group is null)
                    throw new ArgumentNullException($"Group with id: {removeGroupVm.Id} does not exist");
