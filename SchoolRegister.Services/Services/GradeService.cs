...
namespace SchoolRegister.Services.Services
{
    public class GradeService : BaseService, IGradeService
    {
        ...
        public async Task<IEnumerable<Grade>> GetGrades(GetGradesVm checkGradesVm)
        {
            try
            {
                var user = await DbContext.Users.FirstOrDefaultAsync(u => u.Id == checkGradesVm.StudentId);

                if (user == null)
                {
                    throw new ArgumentNullException($"Could not find user with id: {checkGradesVm.StudentId}");
                }
