...
namespace SchoolRegister.Services.Services
{
    public class TeacherService : BaseService, ITeacherService
    {
        ...
        public async void SendMail(SendMailVm SendMailVm)
        {
            try
            {
                if (SendMailVm == null)
                    throw new ArgumentNullException ($"View model parameter is null");
                var sender = await DbContext.Users.FirstOrDefaultAsync(u => u.Id == SendMailVm.FromId);

                if (sender is null)
                    throw new ArgumentNullException("Can't find sender ID");

                var recipient = await DbContext.Users.FirstOrDefaultAsync(u => u.Id == SendMailVm.ToId);
                ...
                if (await userManager.IsInRoleAsync(sender, "Teacher") && await userManager.IsInRoleAsync(recipient, "Parent"))
                {
                    ...
                    var message = new MailMessage(sender.Email, recipient.Email, SendMailVm.Title, SendMailVm.Content);
                    client.Send(message); 
                }
            }
            catch (Exception ex)
